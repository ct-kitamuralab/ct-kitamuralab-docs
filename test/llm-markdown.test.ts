...
describe("llm-markdown: URL helpers", () => {
  ...
  it("pageUrl はネストIDを /id/ 形式へ変換する", async () => {
    const { pageUrl } = await load(DEFAULT_BASE);
    expect(pageUrl("getting-started/coder/vscode-web")).toBe(`${SITE}${DEFAULT_BASE}/getting-started/coder/vscode-web/`);
  });

  it("pageUrl は末尾スラッシュ付き BASE_PATH を正規化する", async () => {
    const { pageUrl } = await load("/custom/");
    expect(pageUrl("getting-started/coder")).toBe(`${SITE}/custom/getting-started/coder/`);
  });
...
describe("llm-markdown: sections", () => {
  ...
  it("sectionFor はネストIDをセクションへ解決する", async () => {
    const { sectionFor } = await load(DEFAULT_BASE);
    expect(sectionFor("getting-started/coder/vscode-web")).toEqual(["利用を始める", "getting-started/"]);
...
describe("llm-markdown: renderMarkdown", () => {
  it("frontmatter と Documentation Index ヘッダーを生成する", async () => {
    ...
    expect(out).toContain(`> Fetch the documentation index at: ${SITE}${DEFAULT_BASE}/getting-started/llms.txt  `);
    expect(out).toContain("# テストページ");
    expect(out).toContain(`Last updated 2026-08-16 | Copy as Markdown | [View as Markdown](${SITE}${DEFAULT_BASE}/getting-started/coder.md)`);
  ...
  it("br / small / div / span / strong タグを除去する", async () => {
    const { renderMarkdown } = await load(DEFAULT_BASE);
    const out = renderMarkdown(entry("getting-started/coder", "a<br/>b<small>c</small><div>d</div><span>e</span><strong>f</strong>"), "2026-08-16");
  ...
  it("タイトル付きリンクの title を保持する", async () => {
    const { renderMarkdown } = await load(DEFAULT_BASE);
    const out = renderMarkdown(entry("getting-started/coder", '[VS Code](vscode-web/ "VS Code Web")'), "2026-08-16");
    expect(out).toContain(`[VS Code](${SITE}${DEFAULT_BASE}/getting-started/coder/vscode-web/ "VS Code Web")`);
  });
...
describe("llm-markdown: renderLlms", () => {
  ...
  it("セクション llms.txt には Optional を含めない", async () => {
    ...
    expect(out).toContain("# getting-started | 喜多村研究室 Docs");
